...
void HelloWorldListener::on_subscription_matched(DDSDataReader *reader,
	const DDS_SubscriptionMatchedStatus &status)
{
	printf("Match status changed. New match count = %d\n",(int)status.current_count);
}
...
extern "C" int subscriber_main(int domainId, int sample_count, char* partition_name)
{
    ...
    subscriberQos.partition.name[0] = partition_name;

    subscriber = participant->create_subscriber(
        subscriberQos, NULL /* listener */, DDS_STATUS_MASK_NONE);
    if (subscriber == NULL) {
    ...
    for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
		...
		{
			lType = (lType+1)%3;
			subscriberQos.partition.name[0][strlen(subscriberQos.partition.name[0])-1] = '0'+lType;
			subscriber->set_qos(subscriberQos);
			printf("Changed partition to %s\n",subscriberQos.partition.name[0]);
		}
