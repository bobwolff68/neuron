...
g++ -g3 -DRTI_UNIX -DRTI_LINUX $DEBUGFLAG -I$NDDSHOME/include -I$NDDSHOME/include/ndds -L$NDDSHOME/lib/i86Linux2.6gcc4.1.1 -ldl -$NDDSCORE -$NDDSC -$NDDSCPP -$RTIMONITORING -o $BINDIR/cfc_pub cfc_publisher.cxx cfcSupport.cxx cfcPlugin.cxx cfc.cxx parsecmd.cpp anyoption/anyoption.cpp cfc_transport_config.cpp

g++ -g3 -DRTI_UNIX -DRTI_LINUX $DEBUGFLAG -I$NDDSHOME/include -I$NDDSHOME/include/ndds -L$NDDSHOME/lib/i86Linux2.6gcc4.1.1 -ldl -$NDDSCORE -$NDDSC -$NDDSCPP -$RTIMONITORING -o $BINDIR/cfc_sub cfc_subscriber.cxx cfcSupport.cxx cfcPlugin.cxx cfc.cxx parsecmd.cpp anyoption/anyoption.cpp cfc_transport_config.cpp
