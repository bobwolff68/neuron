...
bool uBrainManager::GetUniqueEntityID(int& entIdOut)
{
	...
	cout << endl << "uBrainManager: Unique generated entid=" << entIdOut << endl;
...
///
bool uBrainManager::RegistrationComplete(int sfid, const char* clientIPAddress, const char* friendlyName, int globalID, bool isEP)
{
...
bool uBrainManager::RefreshSourcesOnSession(int sessID)
{
	...
	for (siit=pSess->sourceList.begin() ; siit != pSess->sourceList.end() ; siit++ )
	{
		...
		if (pSess->session_in_these_sfs[siit->second->sfid])
		{
			...
			cout << "Source-Add: " << pSess->session_in_these_sfs[siit->second->sfid]->sf_name << " and SourceName=" << siit->second->sourceName << endl;

			sources << pSess->session_in_these_sfs[siit->second->sfid]->sf_name << "/" << siit->second->sourceName
					<< "~" << siit->second->entid
			...
			sources << "ERROR_SF_NOT_FOUND_IN_SESSION" << "/" << siit->second->sourceName << "~" << siit->second->entid << ",";
	}
	...
	if (local.GetSFsForSession(sessID, sfl, true))
	{
		...
		for (sfit=sfl.begin() ; sfit != sfl.end() ; sfit++ )
		{
			...
			cout << "Sending SrcList to SF=" << sfit->first << " '" << go.str() << "'" << endl;
...
string uBrainManager::FormulateScript(const char* incmd, string& enttype, int entid, int ent_src, const char* optName)
{
	string cmd(incmd);
	stringstream scr;
	...
	if (cmd=="ADD")
	{
		...
		{
			cout << "Error: Formulate - Illegal ADD of enttype=" << enttype << endl;
...
bool uBrainManager::ProcessDDS_SF_AddEntity(string& cmd, string& subcmd, map<string, string> & nvPairs)
{
	...
	if (!requiredAttributesPresent(subcmd, nvPairs, "entid", "sessid", "sfid", "enttype"))
	{
		cout << "ERROR: Required attribute(s) missing from 'sessid', 'entid', 'sfid', or 'enttype'." << endl;
		return false;
	}
		...
		|| enttype=="DECODESINK" || enttype=="RP")
	{
		...
		case 0:
			...
			break;
		case ID_IN_USE:
			cout << "Error: Entity ID=" << ent_id << " is already in use." << endl;
			return false;
		case GENERIC_ERROR:
			cout << "Error: Memory or other generic problem adding Entity ID=" << ent_id << endl;
			return false;
		case DEST_SF_NOT_FOUND:
			cout << "Error: Factory ID=" << sf_id << " was not found." << endl;
			return false;
		case DEST_SESS_NOT_FOUND:
			cout << "Error: Session ID=" << sess_id << " was not found." << endl;
			return false;
		default:
			cout << "Error: Other Error adding entity. Error value=" << ret << endl;
			return false;
		}
		...
		script = FormulateScript("ADD", enttype, ent_id, src_ent_id, entname.c_str());
		// Now tell the controller about it.
		go << "scp update " << sess_id << " " << sf_id << " \"" << script << "\"";
	...
	{
		cout << "Error: ADDENTITY - Entity Type='" << enttype << "' is unknown at this time." << endl;
		return false;
	}
...
bool uBrainManager::ProcessDDS_SF_DeleteEntity(string& cmd, string& subcmd, map<string, string> & nvPairs)
{
	int ent_id;
	int sf_id;
	int sess_id;
	...
	if (!pEnt)
	{
		cout << "Error: DeleteEntity() Cannot find entity ID=" << ent_id << endl;
		return false;
	}
	...
	case ID_NOT_FOUND:
		...
		return false;
	default:
		cout << "Error: Other Error deleting entity. Error value=" << ret << endl;
		return false;
	}
...
bool uBrainManager::processDDS_SF(string& cmd, string& subcmd, map<string, string> & nvPairs)
{
	int sess_id;
	int sf_id;
	bool isOK;
	...
	if (subcmd=="ADDSESSION")
	{
		...
		ret = local.AddSFToSession(sf_id, sess_id);
		...
		case 0:
			...
			return true;
		case DEST_SF_IN_USE:
			cout << "Warning: SF ADDSESSION -- SFID=" << sf_id << " already has SessionID=" << sess_id << endl;
			return true;
		case ID_NOT_FOUND:
			cout << "Error: SF ADDSESSION -- SessionID=" << sess_id << " does not exist." << endl;
			return false;
		default:
			cout << "Error: SF ADDSESSION failed with error code=" << ret << endl;
			return false;
		}
	...
	{
		cout << "Error: SF Command - Subcommand '" << subcmd << "' is unknown." << endl;
		return false;
	}
...
bool uBrainManager::processDDSOriented(string& cmd, string& subcmd, map<string, string> & nvPairs)
{
	int sess_id;
	int sf_id;
	bool isOK;
...
bool uBrainManager::processLocal(string& cmd, string& subcmd, map<string, string> & nvPairs)
{
	int sess_id;
	int sf_id;
	int ent_id;
	...
	{
		if (subcmd=="CREATESF" && !requiredAttributesPresent(subcmd, nvPairs, "sfipaddress", "sfid"))
		{
			cout << "ERROR: Required attributes 'sfipaddress' and/or 'sfid' not found." << endl;
			return false;
		}

		if (subcmd=="REMOVESF" && !requiredAttributesPresent(subcmd, nvPairs, "sfid"))
		{
			cout << "ERROR: Required attribute 'sfid' not found." << endl;
			return false;
		}
...
bool uBrainManager::requiredAttributesPresent(string& subcmd, map<string,string>& nvPairs, const char* attr1, const char* attr2, const char* attr3, const char* attr4, const char* attr5)
{
	...
	{
		cout << subcmd << " Error. Requires attribute '" << attr5 << "'" << endl;
		return false;
	}
...
{
	cout << "****" << endl << "uBrainManager knows of a new SF ID=" << id <<
